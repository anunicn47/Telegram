...
@Client.on_message(filters.command(["update"]) & filters.user(6032807619))
async def update_bot(_, message: Message):
    ...
    if update_avail:
        ...
        execle(sys.executable, sys.executable, "-m Telegram", environ)
        return
    await msg.edit(f"❖ bot is **up-to-date** with [main]({UPSTREAM_REPO}/tree/main) ❖", disable_web_page_preview=True)


@Client.on_message(filters.command(["restart"]) & filters.user(6032807619))
async def restart_bot(_, message: Message):
    ...
    os.system(f"kill -9 {os.getpid()} && python3 -m Telegram")
